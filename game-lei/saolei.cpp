...
int main(void)
{
	...
	do {
		...
		case 1:
			printf("开始游戏\\>:\n");
			if (ispropre(wid[hard], len[hard], lei[hard])) {
				start = time(NULL);
				mid = time(NULL);
				game(wid[hard], len[hard], lei[hard], leibiao, panbiao);
			}
			else {
				printf("\t!!!游戏数据有问题!!（可能原因：1：雷的个数超过了雷区的格子数 2：雷区的长，宽超过了限制 ）\n");
...
void game(int wid, int len, int lei,char leibiao,char panbiao)
{
	...
	printboard(leiboard,prinboard, wid, len,' ',panbiao,lei);
	int changeresult = 1;
	do{
	laba1:
		x = 0;
		...
		scanf("%d %d", &y, &x);
		scanf("%*[^\n]%*c");
		if (x > wid || y > len || x == 0 || y == 0 || prinboard[x-1][y-1]) {
			printf("不合法输入！！！ 请重新输入！！！\n\n");
			goto laba1;
		}
...
{
	int a = wid * len;
	if ( lei >= a||  wid <= 0 || wid > WID || len <= 0 || len > LEN) {
		return 0;
	}
	return 1;
}
...
void set(int* wid, int* len, int* lei, int* hard, char* leibiao, char* panbiao)
{
	...
	laba1:
		...
		case 7:
			...
			break;
		case 8:
			beifenset(*(wid + 3), *(len + 3), *(lei + 3), *(hard + 3), *leibiao, *panbiao);
			break;
		case 9:
